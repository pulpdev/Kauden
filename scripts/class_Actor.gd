...
func _ready():
	...
	if Actions:
		Actions.action_finished.connect(on_action_managager_action_finished)
...
func on_request_action_execute(action, params : Array)->void:
	match action:
		ActionRun:
			if States.is_in_state("Free"):
				Actions.set_action("Run", params)
		ActionSprint:
			if States.is_in_state("Free"):
				Actions.set_action("Sprint", params)
		ActionAnimate:
			if States.is_in_state("Free"):
				Actions.set_action("Animate", params)
				States.set_state("ActionStatic")
		ActionStand:
			if States.is_in_state("Free"):
				Actions.set_action("Stand", params)

func on_action_managager_action_finished():
	States.set_state("Free")
