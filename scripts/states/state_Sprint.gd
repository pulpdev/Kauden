...
#
#func enter()->int:
	#if not controller.is_pressing_sprint():
		#return EXIT
...
#func process(delta)->int:
	...
	#controller.player.vector_move = vector_move * controller.player.data.speed_sprint
	#controller.player.Pivot.Model.play_animation(controller.player.data.anim_sprint)
	#return RUN
